...
public interface FoodRepository extends JpaRepository<FoodEntity, UUID> {
    Optional<FoodEntity> findFirstByfoodName(String foodName);

    // 검색어가 있는 경우 (cursor가 없는 경우)
    @Query("SELECT f FROM FoodEntity f " +
        "WHERE LOWER(f.foodName) LIKE LOWER(CONCAT('%', :keyword, '%')) " +
    ...
    @Query("SELECT f FROM FoodEntity f " +
        "WHERE LOWER(f.foodName) LIKE LOWER(CONCAT('%', :keyword, '%')) " +
        "AND (" +
        "  f.createdAt < (SELECT f2.createdAt FROM FoodEntity f2 WHERE f2.id = :cursorId) " +
        "  OR (f.createdAt = (SELECT f2.createdAt FROM FoodEntity f2 WHERE f2.id = :cursorId) AND f.id < :cursorId)" +
        ") " +
        "ORDER BY f.createdAt DESC, f.id DESC")
